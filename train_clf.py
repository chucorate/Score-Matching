...
def train(model, data, *, n_epochs, batch_size, lr, device, seed, checkpoint_path, **kwargs):
    """Entrena 'model' con mini-batches y soporte anti-cortes de energía."""
    set_seed(seed)
    model = model.to(device)
    
    # Split 90/10 para Validación
    val_size = int(0.1 * len(data))
    train_dataset, val_dataset = torch.utils.data.random_split(data, [len(data) - val_size, val_size])
    ...
    print(f"Entrenando en {device} desde la época {start_epoch+1} hasta {n_epochs}...")
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        print(f"Época [{epoch+1}/{n_epochs}] -> Loss (T/V): {history['train_loss'][-1]:.4f}/{history['val_loss'][-1]:.4f} | Acc (T/V): {history['train_acc'][-1]*100:.1f}%/{history['val_acc'][-1]*100:.1f}%")
        
        # GUARDAR RESPALDO DE LA ÉPOCA ACTUAL (Si se corta la luz, no pierdes nada)
        checkpoint_snapshot = {
            "epoch": epoch,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label-dropout", type=float, default=0.0) # Se pide por interfaz externa, se ignora aquí.
    
    args = parser.parse_args()
    
    # Carga de datos
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    mnist_data = datasets.MNIST(root=DEFAULT_DATA_DIR, train=True, download=True, transform=transform)
    ...
    # Ejecución
    historial = train(
        model=clasificador, data=mnist_data, n_epochs=args.epochs, batch_size=args.batch_size,
        lr=args.lr, device=torch.device(args.device), seed=args.seed, checkpoint_path=checkpoint_file
    )
